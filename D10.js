...
*/
const me = { name: "Eddy", surname: "Turpo", age: "34" };
/* ESERCIZIO D
...
const whoIsBigger = (n1, n2) => {
  if (n1 > n2) {
    ...
    console.log("I valori inseriti si equivalgono");
  }
};
whoIsBigger(2, 2);
/* ESERCIZIO 3
...
const onlyLetters = (stringa) => stringa.replace(/\d/g, ""); //.replace(/\d/g,"")
console.log(onlyLetters("I have 5 dogs"));
/* ESERCIZIO 6
  Crea una funzione chiamata "isThisAnEmail" che riceve una stringa come parametro e ritorna true se la stringa è un valido indirizzo email.
*/
const isThisAnEmail = (stringa) => {
  if (stringa.split("").includes("@")) {
    return stringa;
  } else {
    return console.log("inserisci una email valida");
  }
};
console.log(isThisAnEmail("eddy.turpo@hotmail.it"));
/* ESERCIZIO 7
...
const whatDayIsIt = () => {
  ...
  const giornoSettimana = arrayGiorni[giornosettimanaCorrente];
  return `Oggi è: ${giornoSettimana} ${giornoCorrente}/${meseCorrente}/${annoCorrente}`;
};
...
const rollTheDices = (n) => {
  ...
  for (let index = 0; index <= n; index++) {
    let dado = dice();
    console.log(dado);
    arrayNumeriEstratti.push(dado);
    sum += dado;
  }
  return `la somma dei numeri estratti è ${sum} ed i numeri estratti sono i seguenti: ${arrayNumeriEstratti}`;
};
...
const howManyDays = (data) => {
  const dataInizio = new Date(data).getTime();
  const dataCorrente = new Date();
  console.log(dataCorrente);
  ...
  const differenzaDate = dataOggi - dataInizio;
...
const isTodayMyBirthday = (data) => {
  ...
  if (giornoCompleanno === dataOggi && meseCompleanno === meseCorrente) {
    ...
    return "Fai gli esercizi";
  }
};
console.log(isTodayMyBirthday("1990-05-17"));
...
const oggetto2 = { nome: "Eddy", cognome: "Turpo" };

const deleteProp = (obj, stringa) => {
  delete obj[stringa];
  return obj;
};
console.log(deleteProp(oggetto2, "cognome"));
...
const newestMovie = (array) => {
  let filmrecente = { Year: 0 };
  movies.forEach((movie) => {
    let annoFilmcorrente = parseInt(movie.Year);

    if (annoFilmcorrente > filmrecente.Year) {
      filmrecente = movie;
    }
  });
  return filmrecente;
...
/* const onlyTheYears = (movies) => {
  const arrayAnniFilm = [];
  movies.forEach((filmcorrente) => {
    arrayAnniFilm.push(filmcorrente.Year);
  });
  return arrayAnniFilm;
}; */

//OPPURE

const onlyTheYears = (movies) => {
  return movies.map((filmcorrente) => parseInt(filmcorrente.Year));
};
...
const onlyInLastMillennium = (movie) => {
  movies.forEach((movie) => {
    let annoFilmcorrente = parseInt(movie.Year);
    if (annoFilmcorrente < 2000) {
      arrayFilmRecenti.push(movie);
    }
  });
...
const onlyInLastMillennium = (movie) => {
  return movies.filter((movie) => {
    const annoFilmcorrente = parseInt(movie.Year);
    return annoFilmcorrente < 2000;
  });
};
...
const sumAllTheYears = (movies) => {
  const annoFilms = onlyTheYears(movies);
  return annoFilms.reduce(
    (accumulatore, annoCorrente) => accumulatore + annoCorrente,
    0
  );
};
